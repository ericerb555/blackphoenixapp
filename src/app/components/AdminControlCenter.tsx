...
export default function AdminControlCenter({ onClose }: AdminControlCenterProps) {
  ...
  // Handle actions — approving a reel/video automatically publishes it to the landing page
  const handleApprove = async (id: string) => {
    // Find the item being approved
    const allItems = [...reelsVideos];
    const item = allItems.find(i => i.id === id);

    // If it's a reel/video, publish it to the landing page automatically
    if (item && (activeSection === 'reels-videos' || item.data?.duration || item.data?.videoUrl)) {
      try {
        ...
        const reel = {
          ...
          advertiser: {
            name: item.submittedBy || 'Black Phoenix Network',
            type: item.data?.submitterType || 'content',
          },
        ...
        toast.success(`✅ "${item.title}" approved and published to the landing page!`);
      } catch {
