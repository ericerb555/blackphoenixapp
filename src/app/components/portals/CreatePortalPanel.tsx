...
export default function CreatePortalPanel() {
  ...
  const inputClass = 'w-full rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-3 py-2.5 text-sm text-white placeholder-gray-500 outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500';

  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h2 className="flex items-center gap-2 text-xl font-bold text-white"><UserPlus className="h-5 w-5 text-red-600" /> Create a Portal</h2>
      ...
      <form onSubmit={submit} className="space-y-4 rounded-xl border border-[#2A2A2A] bg-[#1A1A1A] p-6">
        <div>
        ...
        <div>
          <label className="mb-1.5 block text-xs font-semibold text-gray-400">Portal type</label>
          <div className="relative"><Building2 className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><select value={form.portalType} onChange={e => set('portalType', e.target.value)} className={`${inputClass} pl-9 appearance-none`}>{PORTAL_OPTIONS.map(o => <option key={o.value} value={o.value} className="bg-[#1A1A1A] text-white">{o.label}</option>)}</select></div>
        </div>

        {/* Trial grant */}
        <div className="rounded-lg border border-[#2A2A2A] bg-white/5 p-4">
          <label className="flex cursor-pointer items-start gap-3">
            <input type="checkbox" checked={fullAccess} onChange={e => setFullAccess(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-[#2A2A2A] text-red-600 focus:ring-red-500" />
            <span>
              <span className="flex items-center gap-1.5 text-sm font-semibold text-white"><ShieldCheck className="h-4 w-4 text-red-600" /> Grant full access to all features</span>
          ...
          {fullAccess && (
            <div className="mt-3 flex items-center gap-2 pl-7">
              ...
              <span className="text-sm text-gray-400">Trial length</span>
              <input type="number" min={1} max={24} value={trialMonths} onChange={e => setTrialMonths(Math.min(24, Math.max(1, Number(e.target.value) || 1)))} className="w-16 rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-2 py-1 text-sm text-white outline-none focus:border-red-500" />
        ...
        <div className="rounded-lg border border-[#2A2A2A] bg-white/5 p-4">
          ...
          <div className="space-y-2">
            <label className="flex cursor-pointer items-center gap-3">
              <input type="checkbox" checked={sendEmail} onChange={e => setSendEmail(e.target.checked)} className="h-4 w-4 rounded border-[#2A2A2A] text-red-600 focus:ring-red-500" />
              <Mail className="h-4 w-4 text-gray-400" />
              <span className="text-sm text-gray-800">Email the secure sign-in link</span>
            </label>
            <label className="flex cursor-pointer items-center gap-3">
              <input type="checkbox" checked={sendSms} onChange={e => setSendSms(e.target.checked)} className="h-4 w-4 rounded border-[#2A2A2A] text-red-600 focus:ring-red-500" />
            ...
            <label className="flex cursor-pointer items-center gap-3">
              <input type="checkbox" checked={generateQr} onChange={e => setGenerateQr(e.target.checked)} className="h-4 w-4 rounded border-[#2A2A2A] text-red-600 focus:ring-red-500" />
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <button type="button" onClick={openPreview} disabled={previewing} className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-5 py-3 text-sm font-bold text-gray-300 transition hover:bg-white/5 disabled:opacity-60 sm:w-auto">
      ...
      {previewHtml !== null && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4" onClick={() => setPreviewHtml(null)}>
          <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl bg-[#1A1A1A] shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-[#2A2A2A] px-5 py-3">
              <div className="min-w-0">
                ...
                <p className="truncate text-sm font-bold text-white">Subject: {previewSubject}</p>
              </div>
              <button onClick={() => setPreviewHtml(null)} className="ml-3 rounded-lg p-1.5 text-gray-400 hover:bg-white/10 hover:text-white"><X className="h-5 w-5" /></button>
            </div>
      ...
      {result && (() => {
        ...
        const border = tone === 'green' ? 'border-green-500/30 bg-green-500/10' : 'border-amber-500/30 bg-amber-500/10';
        const head = tone === 'green' ? 'text-green-400' : 'text-amber-400';
        const bodyText = tone === 'green' ? 'text-green-300' : 'text-amber-300';
        return (
        <div className={`rounded-xl border p-5 ${border}`}>
          ...
          {emailAttempted && result.invitationSent && result.emailProvider === 'supabase' && (
            <div className="mt-3 rounded-lg border border-red-500/30 bg-red-500/10 p-3">
              ...
              <p className="mt-1 text-xs text-red-300">Resend rejected the branded send. Exact reason:</p>
              <pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap break-words rounded bg-black/40 p-2 text-xs text-red-200">
{result.emailFallbackReason || 'Reason not reported by the server (older deploy).'}
              ...
              <button
                ...
                className="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-red-400 hover:underline"
              >
          ...
          {result.qrDataUrl && (
            <div className="mt-4 flex flex-col items-center gap-3 rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] p-4">
              ...
              <a
                ...
                className="inline-flex items-center gap-1.5 rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-4 py-2 text-xs font-semibold text-gray-300 hover:bg-white/5"
              >
