...
import {
  ...
  Calendar, User, Edit2, Send, FileSignature, AlertCircle, Wrench, Package, MessageSquare,
...
type TabType = 'overview' | 'submission' | 'quote' | 'design' | 'materials' | 'contract' | 'invoice' | 'messages';
...
export function ProjectDetailsModal({ item, onClose, onUpdate, onStageChange, onOpenQuoteEditor, initialTab = 'overview' }: ProjectDetailsModalProps) {
  ...
  const tabs = [
    ...
    { id: 'submission', label: 'Customer Files', icon: ImageIcon, available: true, badge: getTotalFilesCount() },
    ...
    { id: 'messages', label: '💬 Message Customer', icon: MessageSquare, available: true },
  ];
...
function CustomerSubmissionTab({ item, onMediaSelect }: any) {
  // Build submission from multiple possible sources
  const wr = item.workRequest || {};
  const media = wr.media_attachments || wr.media || {};

  const rawPhotos: string[] = media.photos || wr.photos || [];
  const rawVideos: string[] = media.videos || wr.videos || [];
  const rawBlueprints: string[] = media.blueprints || wr.blueprints || [];

  const submission = item.submission || (
    (rawPhotos.length > 0 || rawVideos.length > 0 || rawBlueprints.length > 0) ? {
      photos: rawPhotos.map((url: string, i: number) => ({ id: `p${i}`, url, filename: `Photo ${i + 1}` })),
      videos: rawVideos.map((url: string, i: number) => ({ id: `v${i}`, url, filename: `Video ${i + 1}` })),
      documents: [],
      plans: rawBlueprints.map((url: string, i: number) => ({ id: `b${i}`, url, filename: `Blueprint ${i + 1}` })),
    } : null
  );

  if (!submission || (submission.photos?.length === 0 && submission.videos?.length === 0 && submission.plans?.length === 0)) {
    return (
      <div className="space-y-6">
        <div className="text-center py-12 bg-black/30 border border-gray-800 rounded-xl">
          <AlertCircle className="w-12 h-12 text-gray-600 mx-auto mb-3" />
          <p className="text-gray-400 font-medium">No photos or videos uploaded with this request</p>
          <p className="text-gray-600 text-sm mt-1">The customer did not attach any media files</p>
        </div>
        {/* Still show full work request details */}
        <WorkRequestDetailsSection wr={wr} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* AI-Generated Floor Plan & Layout */}
      {item.floorPlan && (
        <div className="bg-gradient-to-br from-orange-500/10 to-purple-500/10 border border-orange-500/30 rounded-xl p-5 space-y-4">
          <h3 className="text-base font-bold text-white flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-orange-400" />
            AI-Generated Floor Plan
            <span className="text-xs font-normal text-orange-400 ml-1">
              {item.floorPlan.source === 'photo-analysis'
                ? `(analyzed ${item.floorPlan.photosAnalyzed} photo${item.floorPlan.photosAnalyzed !== 1 ? 's' : ''})`
                : '(from description)'}
            </span>
          </h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {item.floorPlan.roomType && (
              <div className="bg-black/40 rounded-lg p-3 text-center">
                <p className="text-xs text-gray-500 mb-1">Room Type</p>
                <p className="text-sm font-bold text-white capitalize">{item.floorPlan.roomType}</p>
              </div>
            )}
            {item.floorPlan.estimatedDimensions && (
              <div className="bg-black/40 rounded-lg p-3 text-center">
            ...
            {item.floorPlan.estimatedScope?.complexity && (
              <div className="bg-black/40 rounded-lg p-3 text-center">
                <p className="text-xs text-gray-500 mb-1">Complexity</p>
                <p className={`text-sm font-bold capitalize ${
                  item.floorPlan.estimatedScope.complexity === 'high' ? 'text-red-400' :
                  item.floorPlan.estimatedScope.complexity === 'medium' ? 'text-orange-400' : 'text-green-400'
                }`}>{item.floorPlan.estimatedScope.complexity}</p>
              </div>
            )}
            {item.floorPlan.estimatedScope?.estimatedDays && (
              <div className="bg-black/40 rounded-lg p-3 text-center">
                <p className="text-xs text-gray-500 mb-1">Est. Days</p>
                <p className="text-sm font-bold text-white">{item.floorPlan.estimatedScope.estimatedDays}</p>
              </div>
            )}
          </div>

          {/* Layout description */}
          {item.floorPlan.layout?.description && (
            <p className="text-sm text-gray-300">{item.floorPlan.layout.description}</p>
          )}

          {/* Key features */}
          {item.floorPlan.layout?.keyFeatures?.length > 0 && (
            <div>
              <p className="text-xs font-bold text-gray-500 uppercase mb-2">Key Features</p>
              <div className="flex flex-wrap gap-2">
                {item.floorPlan.layout.keyFeatures.map((f: string, i: number) => (
                  <span key={i} className="text-xs px-2 py-1 bg-orange-500/20 text-orange-300 rounded-full">{f}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {/* Recommendations */}
          {item.floorPlan.layout?.recommendations?.length > 0 && (
            <div>
              <p className="text-xs font-bold text-gray-500 uppercase mb-2">AI Recommendations</p>
              <ul className="space-y-1">
                {item.floorPlan.layout.recommendations.map((r: string, i: number) => (
          ...
          {/* Key tasks */}
          {item.floorPlan.estimatedScope?.keyTasks?.length > 0 && (
            <div>
              <p className="text-xs font-bold text-gray-500 uppercase mb-2">Scope of Work</p>
              <ul className="space-y-1">
                {item.floorPlan.estimatedScope.keyTasks.map((t: string, i: number) => (
                  <li key={i} className="text-xs text-gray-300 flex items-start gap-2">
          ...
          {/* SVG Floor Plan */}
          {item.floorPlan.svgLayout && (
            <div className="bg-white rounded-xl p-4 overflow-auto max-h-64">
              <div
                className="w-full"
                dangerouslySetInnerHTML={{ __html: item.floorPlan.svgLayout }}
              />
            </div>
          )}
        </div>
      ...
      {/* Full work request details always shown below media */}
      <WorkRequestDetailsSection wr={wr} />
    </div>
  );
}

// Full Work Request Details — all the fields the customer filled out
function WorkRequestDetailsSection({ wr }: { wr: any }) {
  if (!wr || !wr.id) return null;

  const budget = wr.budget_range;
  const style = wr.style_preferences;
  const kitchen = wr.kitchen_preferences;
  const design = wr.design_requirements;
  const timeline = wr.timeline;

  return (
    <div className="space-y-4">
      <h3 className="text-base font-bold text-white flex items-center gap-2 border-t border-gray-800 pt-4">
        <FileText className="w-4 h-4 text-orange-400" /> Full Work Request Details
      </h3>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Project Info */}
        <div className="bg-black/40 border border-gray-700 rounded-xl p-4 space-y-2">
          <p className="text-xs font-bold text-orange-400 uppercase tracking-wider mb-3">Project Info</p>
          {wr.project_name && <Row label="Project Name" value={wr.project_name} />}
          {wr.serviceType && <Row label="Service Type" value={wr.serviceType} />}
          {wr.project_type && <Row label="Project Type" value={wr.project_type} />}
          {timeline && <Row label="Timeline" value={timeline} />}
          {wr.priority_level && <Row label="Priority" value={wr.priority_level} />}
          {budget && <Row label="Budget" value={`$${(budget.min||0).toLocaleString()} – $${(budget.max||0).toLocaleString()}`} />}
        </div>

        {/* Property Info */}
        <div className="bg-black/40 border border-gray-700 rounded-xl p-4 space-y-2">
          <p className="text-xs font-bold text-orange-400 uppercase tracking-wider mb-3">Property</p>
          {wr.site_address && <Row label="Address" value={`${wr.site_address}, ${wr.city || ''} ${wr.state || ''} ${wr.zip_code || ''}`} />}
          {wr.property_type && <Row label="Type" value={wr.property_type} />}
        ...
        {/* Design Preferences */}
        {style && (style.primary || style.interior || style.colorPalette) && (
          <div className="bg-black/40 border border-gray-700 rounded-xl p-4 space-y-2">
            <p className="text-xs font-bold text-orange-400 uppercase tracking-wider mb-3">Design Preferences</p>
            {style.primary && <Row label="Arch Style" value={style.primary} />}
            {style.interior && <Row label="Interior Style" value={style.interior} />}
            {style.colorPalette && <Row label="Color Palette" value={style.colorPalette} />}
          </div>
        )}

        {/* Kitchen Preferences */}
        {kitchen && (kitchen.layoutType || kitchen.style || kitchen.countertop) && (
          <div className="bg-black/40 border border-gray-700 rounded-xl p-4 space-y-2">
            <p className="text-xs font-bold text-orange-400 uppercase tracking-wider mb-3">Kitchen Preferences</p>
            {kitchen.layoutType && <Row label="Layout" value={kitchen.layoutType} />}
            {kitchen.style && <Row label="Cabinet Style" value={kitchen.style} />}
            {kitchen.countertop && <Row label="Countertop" value={kitchen.countertop} />}
          </div>
        )}
      </div>
      ...
      {wr.description && (
        <div className="bg-black/40 border border-gray-700 rounded-xl p-4">
          <p className="text-xs font-bold text-orange-400 uppercase tracking-wider mb-2">Customer Description</p>
          <p className="text-gray-300 text-sm leading-relaxed">{wr.description}</p>
        </div>
      )}
    </div>
  );
