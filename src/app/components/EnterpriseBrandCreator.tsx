...
export default function EnterpriseBrandCreator({ 
  ...
  // Handle logo upload — for the primary logo, upload to Supabase Storage so it's
  // permanently accessible from any device via a stable URL (not base64 in localStorage).
  const handleSpecificLogoUpload = (logoType: keyof BrandData) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      // For the primary logo, also push it to Supabase Storage so every device can load it
      if (logoType === 'logoPrimary') {
        ...
        try {
          ...
          if (res.ok && data.logo_url) {
            // Replace the local base64 preview with the permanent storage URL
            setBrandData(prev => ({ ...prev, logoPrimary: data.logo_url }));
            // Keep localStorage in sync too
            const variants = JSON.parse(localStorage.getItem('company_logo_variants') || '{}');
            variants.logo_primary = data.logo_url;
            localStorage.setItem('company_logo_variants', JSON.stringify(variants));
            ...
            toast.success('Logo saved permanently! It will now appear on all devices.', { id: 'logo-upload' });
            console.log('✅ [Logo] Permanent URL:', data.logo_url);
          } else {
            toast.error(`Logo saved locally only: ${data.error || 'upload failed'}`, { id: 'logo-upload' });
          }
        } catch (err) {
        ...
        toast.success(`${logoType.replace('logo', 'Logo ')} uploaded!`);
      }
    };
    reader.onerror = () => {
