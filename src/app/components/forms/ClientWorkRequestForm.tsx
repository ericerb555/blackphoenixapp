...
type FormMode = 'select' | 'quick' | 'detailed';
type FormStep = 'mode-select' | 'subscription' | 'project-type' | 'ai-guide' | 'project' | 'design' | 'style' | 'kitchen' | 'materials' | 'structural' | 'rendering' | 'media' | 'budget' | 'review';
...
export default function ClientWorkRequestForm({ onClose, onProjectCreated }: ClientWorkRequestFormProps) {
  ...
  const [aiConversation, setAiConversation] = useState<Array<{role: 'ai' | 'user', message: string}>>([
    {
      role: 'ai',
      message: "Hi! I'm your Black Phoenix project assistant. I'll walk you through everything we need to get you an accurate quote. Let's start — what's your name and what are you looking to get done?"
    }
  ]);
  ...
  // Get conditional steps based on mode and project category
  const getSteps = (): { id: FormStep; title: string; icon: any; description: string }[] => {
    // Mode selection is always first
    const modeStep = { id: 'mode-select' as FormStep, title: 'How to Start', icon: Target, description: 'Quick or detailed' };

    // ── QUICK mode: only 3 steps after mode select ────────────────────────
    if (formMode === 'quick') {
      return [
        modeStep,
        { id: 'project-type' as FormStep, title: 'Service Type', icon: Target, description: 'What do you need?' },
        { id: 'media' as FormStep, title: 'Photos & Details', icon: Camera, description: 'Show us what you need' },
        { id: 'review' as FormStep, title: 'Submit', icon: CheckCircle, description: 'Review & send' },
      ];
    }

    // ── DETAILED mode: full flow ──────────────────────────────────────────
    if (formMode === 'detailed') {
      const baseSteps = [
        modeStep,
        { id: 'subscription' as FormStep, title: 'Select Plan', icon: DollarSign, description: 'Choose your subscription' },
  ...
  const validateStep = (step: FormStep): boolean => {
    ...
    switch (step) {
      case 'mode-select':
        // Mode must be selected — handled by button click, always valid
        break;
      case 'subscription':
        // In quick mode, skip subscription validation
        if (formMode !== 'quick' && !formData.selectedSubscription) newErrors.selectedSubscription = 'Please select a plan option (Free or Subscription)';
        break;
      case 'project-type':
      ...
      case 'kitchen':
        ...
        break;
      case 'media':
        // For trash/demo projects, photos are required
        if (formData.projectCategory === 'trash_demo' && formData.photos.length === 0) {
          newErrors.photos = 'Please upload at least one photo of items to remove for an accurate quote';
        }
        break;
    }
  ...
  // ── MODE SELECT — first screen customers see ──────────────────────────────
  const renderModeSelectStep = () => (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-white mb-2">How would you like to get started?</h2>
        <p className="text-gray-400">Choose the option that works best for you</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <button
          ...
          className="group relative text-left p-6 bg-gradient-to-br from-orange-600/20 to-red-600/20 border-2 border-orange-500/40 hover:border-orange-500 rounded-2xl transition-all hover:scale-[1.02] hover:shadow-xl hover:shadow-orange-500/20"
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">Quick Request</h3>
              <span className="text-xs text-orange-400 font-semibold">2 minutes</span>
            </div>
          </div>
          <p className="text-sm text-gray-300 mb-4 leading-relaxed">
            Tell us what you need and upload a few photos. Perfect for straightforward jobs — we'll follow up with questions if needed.
          </p>
          <ul className="space-y-2 text-sm text-gray-400">
            <li className="flex items-center gap-2"><span className="text-green-400">✓</span> Choose service type</li>
            <li className="flex items-center gap-2"><span className="text-green-400">✓</span> Add photos & location</li>
            <li className="flex items-center gap-2"><span className="text-green-400">✓</span> Submit — we handle the rest</li>
          </ul>
        ...
        <button
          ...
          className="group relative text-left p-6 bg-gradient-to-br from-blue-600/20 to-purple-600/20 border-2 border-blue-500/40 hover:border-blue-500 rounded-2xl transition-all hover:scale-[1.02] hover:shadow-xl hover:shadow-blue-500/20"
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <span className="text-xs text-blue-400 font-semibold">Most accurate quote</span>
            </div>
          </div>
          <p className="text-sm text-gray-300 mb-4 leading-relaxed">
            Walk through every detail with our AI assistant — materials, design preferences, budget, and more. Get the most precise quote possible.
          </p>
          <ul className="space-y-2 text-sm text-gray-400">
            <li className="flex items-center gap-2"><span className="text-blue-400">✓</span> AI-guided walkthrough</li>
            <li className="flex items-center gap-2"><span className="text-blue-400">✓</span> Design & material preferences</li>
            <li className="flex items-center gap-2"><span className="text-blue-400">✓</span> Video walkthrough option</li>
            <li className="flex items-center gap-2"><span className="text-blue-400">✓</span> More accurate quote</li>
          </ul>
          <div className="absolute bottom-5 right-5 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <p className="text-center text-xs text-gray-600">
        Not sure? Start with Quick — you can always add more details later.
      </p>
    </div>
  );
  ...
  // Subscription Selection Step - FIRST STEP for new customers
  const renderSubscriptionStep = () => {
    // Load cohort type from localStorage (set by "Get a Free Quote" button)
    const cohortType = localStorage.getItem('quote_request_cohort') || 'handyman';

    // Load sections from localStorage override or use default config (same as DirectoryLandingPage)
    let sections = DIRECTORY_SECTIONS;
    const override = localStorage.getItem('directory_sections_override');
    if (override) {
      try {
        sections = JSON.parse(override);
      } catch (e) {
        console.error('Failed to parse directory sections override:', e);
      }
    }

    const section = sections.find(s => s.cohortType === cohortType);

    if (!section || !section.subscriptionPlans) {
      // If no subscription plans, skip to next step
      return (
        <div className="space-y-8">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-white mb-2">Welcome!</h2>
            <p className="text-gray-400">Let's get started with your project</p>
            <button
              onClick={() => setCurrentStep('project-type')}
              className="mt-6 px-8 py-3 bg-orange-600 hover:bg-orange-500 text-white rounded-xl font-semibold"
            >
              Continue
            </button>
          </div>
    ...
    return (
      <div className="space-y-8">
        <div className="text-center flex flex-col items-center">
          <div className={`inline-flex items-center justify-center w-16 h-16 rounded-xl bg-gradient-to-br ${section.gradient} mb-4`}>
            <section.icon className="w-8 h-8 text-white" />
          </div>
          <h2 className="text-2xl font-bold text-white mb-2">{section.title}</h2>
          <p className="text-gray-400 mb-6">{section.subtitle}</p>
          <p className="text-gray-300 text-sm max-w-2xl mx-auto text-center">{section.description}</p>
        </div>

        <div>
          <h3 className="text-xl font-bold text-white mb-4 text-center">Choose Your Subscription Plan</h3>
          <p className="text-gray-400 text-sm text-center mb-6">Select a plan to get started, or choose pay-as-you-go with no subscription.</p>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 max-w-7xl mx-auto">
            {/* Free / No Subscription Option */}
            <div
              className={`relative p-6 rounded-xl border-2 transition-all cursor-pointer ${
                formData.selectedSubscription?.cohortId === 'no-subscription'
                  ? `border-green-500 bg-green-500/10 shadow-lg shadow-green-500/20`
                  : 'border-[#2A2A2A] bg-[#1A1A1A] hover:border-green-500/50'
              }`}
              onClick={() => {
                updateFormData('selectedSubscription', {
                  cohortType: section.cohortType,
                  cohortId: 'no-subscription',
            ...
            >
              <div className="text-center">
                ...
                <div className="mb-6">
                  <span className="text-green-400 font-semibold">Pay as you go</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" />
                  <span className="text-gray-300">Standard hourly rates</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" />
                  <span className="text-gray-300">Get quotes for projects</span>
                </div>
              </div>
            ...
            {section.subscriptionPlans.map((plan, index) => {
              const isSelected = formData.selectedSubscription?.cohortId === plan.cohortId;
              ...
              return (
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <div className="mb-6">
                      <span className="text-orange-400 font-semibold">{plan.hours}</span>
                    </div>
                  </div>
        ...
        <div className="text-center">
          <button
            onClick={() => setCurrentStep('project-type')}
            disabled={!formData.selectedSubscription}
            className="px-8 py-3 bg-orange-600 hover:bg-orange-500 disabled:bg-gray-700 disabled:cursor-not-allowed text-white rounded-xl font-semibold transition-all"
          >
            Continue to Project Details
          </button>
          {!formData.selectedSubscription && (
            <p className="text-gray-400 text-sm mt-2">Please select a plan option to continue (Free or Subscription)</p>
          )}
        </div>
      </div>
    );
  ...
  const renderProjectTypeStep = () => (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <button
          onClick={() => {
            updateFormData('projectCategory', 'trash_demo');
            updateFormData('serviceType', 'Trash & Demo Removal');
          }}
          className={`p-6 rounded-xl border-2 transition-all text-left ${
            formData.projectCategory === 'trash_demo'
              ? 'border-orange-500 bg-orange-500/10'
              : 'border-[#2A2A2A] bg-[#1A1A1A] hover:border-orange-500/50'
          }`}
        >
          <Trash2 className="w-10 h-10 text-orange-400 mb-3" />
          <h3 className="text-lg font-bold text-white mb-2">Trash & Demo Removal</h3>
          <p className="text-sm text-gray-400">Demolition, junk removal, debris hauling - upload photos for quote</p>
        </button>
      </div>
  ...
  // Send a message to the AI guide and apply any field updates it returns
  const sendAiMessage = async (message: string) => {
    if (!message.trim() || aiIsThinking) return;
    const newConversation = [...aiConversation, { role: 'user' as const, message }];
    ...
    try {
      ...
      // Apply extracted field updates directly to the form
      if (data.fieldUpdates && Object.keys(data.fieldUpdates).length > 0) {
        setFormData((prev: any) => ({ ...prev, ...data.fieldUpdates }));
      }

      setAiMissingFields(data.missingFields || []);
      setAiComplete(data.complete || false);
      setAiConversation(prev => [...prev, { role: 'ai', message: data.message }]);
    } catch (err) {
      setAiConversation(prev => [...prev, {
        role: 'ai',
        message: "Sorry, I'm having trouble connecting right now. You can fill in the form fields manually and click Continue when ready."
      }]);
    } finally {
  ...
  // Quick-reply suggestions based on missing fields
  const getQuickReplies = (): string[] => {
    if (aiComplete) return [];
    const missing = aiMissingFields;
    if (missing.includes('Timeline')) return ['As soon as possible', 'Within 1 month', 'Within 2-3 months', 'Flexible timing'];
    if (missing.includes('Budget range')) return ['Under $5,000', '$5,000–$15,000', '$15,000–$50,000', 'Over $50,000'];
    if (missing.includes('Property type')) return ['Single family home', 'Condo / apartment', 'Multi-family', 'Commercial'];
    if (missing.includes('Priority level')) return ['Urgent', 'High priority', 'Normal', 'Low – no rush'];
    if (missing.includes('Kitchen layout type')) return ['L-shaped', 'U-shaped', 'Galley / straight', 'Island kitchen'];
    return [];
  };
  ...
  const renderAIGuideStep = () => {
    ...
    const totalFields = aiMissingFields.length + (13 - aiMissingFields.length); // approx 13 key fields
    const filledCount = 13 - aiMissingFields.length;
    const progress = Math.min(100, Math.round((filledCount / 13) * 100));

    return (
      <div className="space-y-4">
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">AI Project Assistant</h2>
          <p className="text-gray-400 text-sm">I'll ask you a few questions to fill out your request automatically.</p>
        </div>

        {/* Progress bar */}
        {aiMissingFields.length > 0 || aiComplete ? (
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-3">
            <div className="flex justify-between text-xs text-gray-400 mb-2">
              <span>Form completion</span>
              <span className={aiComplete ? 'text-green-400 font-semibold' : 'text-orange-400'}>{progress}%</span>
            </div>
            <div className="h-2 bg-[#0A0A0A] rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-500 ${aiComplete ? 'bg-green-500' : 'bg-orange-500'}`}
                style={{ width: `${progress}%` }}
              />
            </div>
            {aiMissingFields.length > 0 && (
              <p className="text-xs text-gray-500 mt-2">Still needed: {aiMissingFields.slice(0, 4).join(', ')}{aiMissingFields.length > 4 ? ` +${aiMissingFields.length - 4} more` : ''}</p>
            )}
          </div>
        ) : null}

        {/* Chat window */}
        <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-4 h-72 overflow-y-auto space-y-3 flex flex-col">
          {aiConversation.map((msg, idx) => (
            <div key={idx} className={`flex gap-2 ${msg.role === 'ai' ? 'justify-start' : 'justify-end'}`}>
              ...
              <div className={`max-w-[82%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                msg.role === 'ai'
                  ? 'bg-[#0A0A0A] border border-[#2A2A2A] text-gray-200 rounded-tl-none'
                  : 'bg-orange-600/30 border border-orange-500/30 text-white rounded-tr-none'
              }`}>
                {msg.message}
              </div>
              {msg.role === 'user' && (
          ...
          {aiIsThinking && (
            <div className="flex gap-2 justify-start">
              ...
              <div className="bg-[#0A0A0A] border border-[#2A2A2A] px-4 py-3 rounded-2xl rounded-tl-none flex gap-1 items-center">
        ...
        {/* Quick reply chips */}
        {quickReplies.length > 0 && !aiIsThinking && (
          <div className="flex flex-wrap gap-2">
            {quickReplies.map((reply) => (
              <button
                key={reply}
                onClick={() => sendAiMessage(reply)}
                className="px-3 py-1.5 text-xs bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/30 text-orange-300 rounded-full transition-colors"
              >
                {reply}
              </button>
            ))}
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && !e.shiftKey && userInput.trim()) {
                e.preventDefault();
                sendAiMessage(userInput.trim());
              }
            }}
            placeholder={aiComplete ? "All set! Click Continue below." : "Type your answer or use a quick reply above..."}
            disabled={aiIsThinking || aiComplete}
            className="flex-1 px-4 py-3 bg-[#0A0A0A] border border-[#2A2A2A] rounded-xl text-white placeholder-gray-500 focus:border-orange-500 focus:outline-none disabled:opacity-50 text-sm"
          />
          <button
            onClick={() => sendAiMessage(userInput.trim())}
            disabled={!userInput.trim() || aiIsThinking || aiComplete}
            className="px-5 py-3 bg-orange-600 hover:bg-orange-500 disabled:bg-gray-700 disabled:cursor-not-allowed rounded-xl transition flex items-center gap-2 text-white font-semibold text-sm"
          >
        ...
        {aiComplete && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4 flex items-center gap-3">
            ...
            <p className="text-sm text-green-300 font-medium">All set! Your form is filled in — click <strong>Continue</strong> to review and submit.</p>
          </div>
        )}
      </div>
    );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400">
          {formData.projectCategory === 'trash_demo'
            ? 'Upload photos of items to remove - Required for accurate quote'
            : 'Upload media to help us understand your space (optional)'}
        </p>
      </div>

      {formData.projectCategory === 'trash_demo' && (
        <div className="bg-gradient-to-br from-red-600/20 to-orange-600/20 border-2 border-red-500/30 rounded-xl p-6">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-xl bg-red-600 flex items-center justify-center flex-shrink-0">
              <Camera className="w-6 h-6 text-white" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-white mb-2">Photos Required for Trash & Demo Quote</h3>
              <p className="text-gray-300 text-sm mb-3">
                To provide you with an accurate quote, please upload clear photos showing:
              </p>
              <ul className="text-sm text-gray-300 space-y-1 mb-3">
                <li className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" />
                  <span>Full view of items or debris to be removed</span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" />
                  <span>Close-up shots showing material type and condition</span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" />
                  <span>Access points (doors, hallways, stairs)</span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" />
                  <span>Any special considerations (tight spaces, heavy items)</span>
                </li>
              </ul>
              <p className="text-orange-300 text-sm font-semibold">
                💡 Tip: More photos = more accurate quote! Include photos from multiple angles.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-6">
        <h4 className="text-white font-semibold mb-4">
          {formData.projectCategory === 'trash_demo' ? 'Upload Photos of Items to Remove' : 'Upload Photos'}
          {formData.projectCategory === 'trash_demo' && (
            <span className="text-red-400 ml-2 text-sm">*Required</span>
          )}
        </h4>
        {/* Hidden inputs — one for camera, one for file gallery */}
        <input
          ref={photoInputRef}
          type="file"
          multiple
        ...
        {/* Two clear buttons — Take Photo (camera) and Choose from Library */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => document.getElementById('camera-capture-input')?.click()}
            className={`w-full px-6 py-4 bg-[#0A0A0A] border-2 border-dashed rounded-xl transition flex items-center justify-center gap-2 ${
              formData.projectCategory === 'trash_demo'
                ? 'border-red-500/50 hover:border-red-500 text-red-400 hover:text-red-300'
                : 'border-orange-500/50 hover:border-orange-500 text-orange-400 hover:text-orange-300'
            }`}
          >
          ...
          <button
            ...
            className={`w-full px-6 py-4 bg-[#0A0A0A] border-2 border-dashed rounded-xl transition flex items-center justify-center gap-2 ${
              formData.projectCategory === 'trash_demo'
                ? 'border-red-500/50 hover:border-red-500 text-red-400 hover:text-red-300'
                : 'border-[#2A2A2A] hover:border-orange-500/50 text-gray-400 hover:text-white'
            }`}
          >
        ...
        {/* Mobile camera permission tip */}
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
          <p className="text-xs text-blue-300 flex items-start gap-2">
            <Info className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
            <span>
              <span className="font-semibold">On mobile:</span> Tap <strong>"Take Photo"</strong> to open your camera directly.
              If your browser asks for camera access, tap <strong>"Allow"</strong>.
              If you previously denied it, go to your phone's <strong>Settings → Browser → Camera → Allow</strong>.
            </span>
          </p>
        </div>
        {formData.photos.length > 0 && (
          <p className="text-sm text-green-400 mt-2">
            ...
            {formData.projectCategory === 'trash_demo' && formData.photos.length >= 3 && (
              <span className="ml-2">✓ Good coverage!</span>
            )}
          </p>
        )}
        {formData.projectCategory === 'trash_demo' && formData.photos.length > 0 && formData.photos.length < 3 && (
          <p className="text-sm text-orange-400 mt-2">
            💡 Upload at least 3 photos for best quote accuracy
          </p>
        )}
        {errors.photos && (
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#0A0A0A] border border-[#2A2A2A] rounded-2xl max-w-5xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className={`px-6 py-4 border-b border-[#2A2A2A] ${currentStep === 'mode-select' ? 'hidden' : ''}`}>
