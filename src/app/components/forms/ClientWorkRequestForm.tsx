...
type FormStep = 'subscription' | 'project-type' | 'ai-guide' | 'project' | 'design' | 'style' | 'kitchen' | 'materials' | 'structural' | 'rendering' | 'media' | 'budget' | 'review';
...
export default function ClientWorkRequestForm({ onClose, onProjectCreated }: ClientWorkRequestFormProps) {
  const { user } = useAuth();
  const [currentStep, setCurrentStep] = useState<FormStep>('subscription');
  ...
  const getSteps = (): { id: FormStep; title: string; icon: any; description: string }[] => {
    ...
    // Default: show subscription and project-type only
    return baseSteps;
  };
  ...
  const validateStep = (step: FormStep): boolean => {
    ...
    switch (step) {
      case 'subscription':
        if (!formData.selectedSubscription) newErrors.selectedSubscription = 'Please select a plan option (Free or Subscription)';
        break;
      case 'project-type':
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-6">
        ...
        >
          ...
          {formData.projectCategory === 'trash_demo'
            ? 'Click to upload photos for quote'
