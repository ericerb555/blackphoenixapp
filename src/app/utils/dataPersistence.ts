  ...
  private async restoreFromDatabase() {
    try {
      ...
      if (error.name === 'AbortError') {
        console.warn('[DataPersistence] Restore request timeout - continuing with existing data');
      } else {
        console.error('[DataPersistence] Failed to restore from database:', error);
      }
    }
  }
  ...
  private monitorStorage() {
    ...
    this.storageListener = (e: StorageEvent) => {
      if (!e.key) {
        ...
        console.error('🚨 [DataPersistence] DETECTED localStorage.clear()! Attempting recovery...');
        this.restoreFromDatabase();
        return;
      }

      if (CRITICAL_KEYS.includes(e.key) && !e.newValue && e.oldValue) {
        console.error(`🚨 [DataPersistence] CRITICAL DATA DELETED: ${e.key}! Attempting recovery...`);
        this.restoreFromDatabase();
      }
    };

    window.addEventListener('storage', this.storageListener);
    console.log('👁️ [DataPersistence] Storage monitoring active');
  }
...
if (typeof window !== 'undefined') {
  ...
  Promise.race([
    ...
    console.warn('[DataPersistence] Initialization skipped:', err.message);
