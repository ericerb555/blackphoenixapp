 ...
 * COMPLETE material lists - every screw, shim, fastener, consumable
 */
...
function generateGenericQuote(item: PipelineItem) {
  ...
  const materials = [
    ...
    { id: `mat-${Date.now()}-2`, name: 'Fasteners & Hardware', description: 'Screws, nails, bolts, anchors', quantity: 1, unit: 'kit', unitCost: estimatedValue * 0.05, totalCost: estimatedValue * 0.05, supplier: 'Fastener Supply', category: 'Fasteners', visible: true },
    { id: `mat-${Date.now()}-3`, name: 'Adhesives & Sealants', description: 'Construction adhesive, caulk, sealants', quantity: 1, unit: 'kit', unitCost: estimatedValue * 0.03, totalCost: estimatedValue * 0.03, supplier: 'General Supply', category: 'Adhesives', visible: true },
    { id: `mat-${Date.now()}-4`, name: 'Installation Supplies', description: 'Drop cloths, tape, protective materials', quantity: 1, unit: 'kit', unitCost: estimatedValue * 0.02, totalCost: estimatedValue * 0.02, supplier: 'General Supply', category: 'Installation Materials', visible: true },
    { id: `mat-${Date.now()}-5`, name: 'Finishing Materials', description: 'Final finishing and touch-up materials', quantity: 1, unit: 'kit', unitCost: estimatedValue * 0.05, totalCost: estimatedValue * 0.05, supplier: 'General Supply', category: 'Finishing', visible: true },
  ];

  const labor = [
    { id: `lab-${Date.now()}-1`, role: 'Site Preparation', description: 'Prepare work area, protect surroundings', hours: Math.round(estimatedValue * 0.002), hourlyRate: 75, totalCost: Math.round(estimatedValue * 0.002) * 75, visible: true },
    { id: `lab-${Date.now()}-2`, role: 'Demolition & Removal', description: 'Remove existing materials, haul away debris', hours: Math.round(estimatedValue * 0.003), hourlyRate: 75, totalCost: Math.round(estimatedValue * 0.003) * 75, visible: true },
    { id: `lab-${Date.now()}-3`, role: 'Primary Installation', description: `Professional installation of ${item.serviceType.toLowerCase()}`, hours: Math.round(estimatedValue * 0.005), hourlyRate: 85, totalCost: Math.round(estimatedValue * 0.005) * 85, visible: true },
    { id: `lab-${Date.now()}-4`, role: 'Finishing Work', description: 'Final finishing, touch-ups, quality control', hours: Math.round(estimatedValue * 0.002), hourlyRate: 75, totalCost: Math.round(estimatedValue * 0.002) * 75, visible: true },
    { id: `lab-${Date.now()}-5`, role: 'Cleanup & Final Inspection', description: 'Clean work area, final walkthrough', hours: Math.round(estimatedValue * 0.001), hourlyRate: 65, totalCost: Math.round(estimatedValue * 0.001) * 65, visible: true },
    { id: `lab-${Date.now()}-6`, role: 'Project Management', description: 'Coordinate work, manage timeline, quality control', hours: Math.round(estimatedValue * 0.002), hourlyRate: 75, totalCost: Math.round(estimatedValue * 0.002) * 75, visible: true },
  ];
