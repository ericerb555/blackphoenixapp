        ...
        <div className="w-80 bg-[#0F0F0F] border-r border-[#2A2A2A] flex flex-col overflow-hidden">
          ...
          <div className="p-4 border-b border-[#2A2A2A]">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xs font-bold text-gray-400 uppercase">Upcoming Jobs</h3>
              <span className="text-xs text-orange-400 font-medium">{jobs.filter(j => j.status !== 'completed').length}</span>
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-4">
              <h3 className="text-xs font-bold text-gray-400 uppercase mb-3">Positions / Skills</h3>
        ...
        <div className="border-t-4 border-[#ea580c]/30 bg-[#0F0F0F]">
          <div className="h-14 bg-[#1A1A1A] border-b border-[#2A2A2A] flex items-center justify-between px-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500 ml-2">Integrated Event & Appointment Scheduling</span>
            </div>
          </div>
                      ...
                      {jobs.filter(j => j.status === 'pending' || j.assignedEmployees.length === 0).map(job => (
                        <div key={job.id} className="flex items-center justify-between p-3 bg-[#1A1A1A] rounded-lg border border-[#2A2A2A]">
                          <div>
                            ...
                            <p className="text-xs text-gray-400">{job.requiredSkills.join(', ')}</p>
                          </div>
      ...
      <div>
        {employees.map((employee: any) => (
          <div key={employee.id} className="flex border-b border-[#2A2A2A] hover:bg-[#1A1A1A]/50">
            ...
            <div className="w-48 p-3 border-r border-[#2A2A2A]">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-orange-600 to-orange-700 flex items-center justify-center text-white text-xs font-bold">
                  {employee.avatar}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-white truncate">{employee.name}</div>
                  <div className="text-xs text-gray-500">{employee.hoursScheduled}h / {employee.maxHours}h</div>
                </div>
              </div>
            </div>
                ...
                <div key={i} className="flex-1 min-w-[140px] p-2 border-r border-[#2A2A2A]">
                  {dayShifts.map((shift: any) => {
                    ...
                    return (
                      <div
                        key={shift.id}
                        className="mb-1 p-2 rounded text-xs cursor-pointer hover:opacity-80 transition"
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{job.id} • {job.customer}</p>
                  </div>
                </div>
      ...
      <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-[#0F0F0F] rounded-lg">
              <div className="text-xs text-gray-400 mb-1">Estimated Hours</div>
              <div className="text-sm font-medium text-white">{job.estimatedHours} hours</div>
            </div>
            
            <div className="p-4 bg-[#0F0F0F] rounded-lg">
              <div className="text-xs text-gray-400 mb-1">Job Value</div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {job.requiredSkills.map((skill: string, i: number) => (
                <span key={i} className="px-3 py-1 bg-[#2A2A2A] text-gray-300 rounded text-xs">
                  {skill}
                </span>
              ))}
            </div>
