...
export default function SubcontractorApplication({ onNavigate }: SubcontractorApplicationProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const existingApplications = JSON.parse(localStorage.getItem('subcontractor_applications') || '[]');
      existingApplications.push(subcontractorData);
      localStorage.setItem('subcontractor_applications', JSON.stringify(existingApplications));

      // Try to submit to backend
      try {
        const response = await fetch(`${API_BASE}/subcontractor-applications`, {
          method: 'POST',
          headers: {
        ...
        console.log('ℹ️ Application saved locally (backend unavailable)');
      }

      toast.success('Application submitted successfully! We\'ll review it and contact you soon.');

      // Navigate back or to confirmation
      if (onNavigate) {
        setTimeout(() => {
          onNavigate('contractor-network-landing-page');
        }, 2000);
      }
    } catch (error) {
