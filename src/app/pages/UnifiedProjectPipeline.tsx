...
export default function UnifiedProjectPipeline() {
  ...
  useEffect(() => {
    const loadPipelineItems = async () => {
      ...
      // Step 2: Fetch real work requests with auto-generated quotes from KV store
      try {
        const url = `https://${projectId}.supabase.co/functions/v1/make-server-57095a78/kv/get-by-prefix?prefix=pipeline:`;
        console.log('[Pipeline] Fetching work requests from KV store...');
        ...
        if (response.ok) {
          ...
          console.log('[Pipeline] KV response:', data);

          if (data.values && Array.isArray(data.values) && data.values.length > 0) {
            const kvItems: PipelineItem[] = data.values.map((item: any) => ({
              id: item.id,
              itemNumber: item.id.toUpperCase().replace('PROJ-', 'WR-'),
              stage: item.stage || 'quote-draft',
              customerName: item.customer || item.clientInfo?.name || 'Unknown',
              customerEmail: item.contact?.email || item.clientInfo?.email || '',
              customerPhone: item.contact?.phone || item.clientInfo?.phone || '',
              ...
              quote: item.quote ? {
                id: `qt-${item.id}`,
                quoteNumber: `Q-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 999) + 1).padStart(3, '0')}`,
                ...
                taxRate: 0.08,
                taxAmount: item.quote.subtotals?.tax || 0,
                totalCost: item.quote.total || 0,
                generatedAt: item.quote.generatedAt || new Date().toISOString(),
                approvalStatus: (item.quote.status || 'draft') as 'pending' | 'approved' | 'rejected' | 'revised'
              } : undefined
            }));

            console.log('[Pipeline] ✅ Loaded', kvItems.length, 'work requests from KV store');

            // Merge KV items with local demo items (KV items first so they appear at top)
            const mergedItems = [...kvItems, ...localItems];
            setItems(mergedItems);
            setIsLoading(false);
            return;
          }
        } else {
          console.log('[Pipeline] KV fetch failed, using localStorage only');
        }
      } catch (error) {
        console.error('[Pipeline] Error loading from KV store:', error);
      }

      // If KV fetch failed or returned no items, use localStorage only
      setItems(localItems);
      setIsLoading(false);
    };
