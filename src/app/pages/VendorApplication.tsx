...
export default function VendorApplication({ onNavigate }: VendorApplicationProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const healthCheck = null; // Assume offline
      ...
      if (!healthCheck || !healthCheck.ok) {
        // Server is offline - save locally
        console.log('[VendorApp] Server offline - saving application locally');
        
        const tempId = `VENDOR-APP-${Date.now()}`;
        const application = { id: tempId, ...vendorData, _offline: true };
        
        const existingApps = localStorage.getItem('vendor_applications_pending') || '[]';
        const apps = JSON.parse(existingApps);
        apps.push(application);
        localStorage.setItem('vendor_applications_pending', JSON.stringify(apps));
        
        toast.success('Application saved! It will be submitted when the server is online.', {
          duration: 5000
        });
      ...
      // Server is online - submit normally
      ...
      if (!response || !response.ok) {
        ...
        toast.warning('Application saved locally - will sync when server is available');
        if (onNavigate) onNavigate('login');
        return;
      }
      ...
      console.error('[VendorApp] Error:', error);
      ...
      toast.info('Application saved locally - will be submitted when possible');
      if (onNavigate) onNavigate('login');
    } finally {
      setSubmitting(false);
    }
  ...
  const steps = [
    ...
    { number: 4, title: 'API Integration', icon: Code }
  ];
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <div className="flex items-start gap-3 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg mb-6">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-blue-400 mb-1">API Integration (Optional)</h3>
                  <p className="text-sm text-gray-300">
                    If you have an existing API or inventory management system, you can provide integration details here. 
                    This allows automated order processing, real-time inventory updates, and seamless data synchronization. 
                    You can also add or update this information later from your vendor portal.
                  </p>
                </div>
              </div>
              ...
              {formData.hasApiIntegration && (
                <div className="space-y-6 border-l-2 border-orange-500/30 pl-6">
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">Your API authentication credentials (stored securely)</p>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">Link to your API documentation for reference</p>
                  </div>
