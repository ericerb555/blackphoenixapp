...
import {
  ...
  Upload, X, Plus, Image, Video, File, Edit2, Save, AlertTriangle, Paperclip, Play,
              ...
              <div className={
                ...
                {filteredAndSortedJobs.map(job => (
                  ...
                  >
                    ...
                    <div className="px-5 pt-4 pb-2 border-b border-[#2A2A2A] space-y-3">
                      <p className="text-gray-300 text-sm leading-relaxed">{job.description}</p>
                      {job.requirements?.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {job.requirements.map((r, i) => (
                            <span key={i} className="px-2.5 py-1 bg-[#0A0A0A] border border-[#2A2A2A] rounded-full text-xs text-gray-400">{r}</span>
                          ))}
                        </div>
                      )}
                      {job.attachments?.length > 0 && (
                        <div>
                          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                            <Paperclip className="w-3.5 h-3.5" /> Attachments ({job.attachments.length})
                          </p>
                          <div className="flex flex-wrap gap-2">
                            {job.attachments.map((a, i) => {
                              const isImg = /\.(jpg|jpeg|png|gif|webp)$/i.test(a);
                              const isVid = /\.(mp4|mov|webm|avi)$/i.test(a);
                              return (
                                <div key={i} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0A0A0A] border border-[#2A2A2A] rounded-lg text-xs text-gray-300 hover:border-orange-500/40 transition cursor-pointer">
                                  {isImg ? <Image className="w-3.5 h-3.5 text-blue-400" /> : isVid ? <Video className="w-3.5 h-3.5 text-orange-400" /> : <File className="w-3.5 h-3.5 text-gray-400" />}
                                  {a}
                                </div>
                              );
                            })}
