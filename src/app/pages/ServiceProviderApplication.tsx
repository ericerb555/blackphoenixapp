...
export default function ServiceProviderApplication({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/service-providers/register`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        toast.success('Application submitted successfully! We\'ll review it and get back to you within 24-48 hours.');
