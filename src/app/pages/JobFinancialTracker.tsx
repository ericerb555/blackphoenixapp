  ...
  const columns: DataTableColumn<Job>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col gap-2">
          <span className={`px-2 py-1 rounded-full text-xs font-semibold text-white ${getStatusColor(row.status)}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-400 font-semibold">{row.completionPercentage}%</span>
          </div>
        </div>
      ),
      align: 'center',
    },
    {
      ...
      render: (row) => (
        <div>
          ...
          <p className="text-xs text-gray-400">of ${row.estimatedRevenue.toLocaleString()}</p>
        </div>
      ),
      align: 'right',
    },
    {
      ...
      render: (row) => (
        <div>
          ...
          <p className="text-xs text-gray-400">of ${row.estimatedCosts.toLocaleString()}</p>
        </div>
      ),
      align: 'right',
    },
      ...
      render: (row) => {
        ...
        return (
          <div>
            ...
            <p className="text-xs text-gray-400">{row.profitMargin.toFixed(1)}% margin</p>
          </div>
        );
      },
      align: 'right',
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <button
            onClick={() => window.location.href = `/job-financial-detail?id=${row.id}`}
