  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-57095a78/territory-applications`,
        {
          method: 'POST',
          headers: {
      ...
      if (data.success) {
        setSubmitted(true);
        toast.success('Territory application submitted successfully!');
      } else {
        // Fallback to localStorage if server is offline
        const applications = JSON.parse(localStorage.getItem('territory_applications') || '[]');
        applications.push({
          id: `APP-${Date.now()}`,
          ...formData,
          status: 'pending',
        ...
        localStorage.setItem('territory_applications', JSON.stringify(applications));
        setSubmitted(true);
        toast.success('Application submitted (offline mode)');
      }
    } catch (error) {
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  <div className="w-6 h-6 bg-[#ea580c]/20 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                    <span className="text-xs font-bold text-[#ea580c]">1</span>
                  </div>
                  <div>
                ...
                <div className="flex items-start gap-3">
                  <div className="w-6 h-6 bg-[#ea580c]/20 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                    <span className="text-xs font-bold text-[#ea580c]">2</span>
                  </div>
                  <div>
                ...
                <div className="flex items-start gap-3">
                  <div className="w-6 h-6 bg-[#ea580c]/20 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                    <span className="text-xs font-bold text-[#ea580c]">3</span>
                  </div>
                  <div>
                ...
                <div className="flex items-start gap-3">
                  <div className="w-6 h-6 bg-[#ea580c]/20 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                    <span className="text-xs font-bold text-[#ea580c]">4</span>
                  </div>
                  <div>
            ...
            {[1, 2, 3, 4].map((stepNum) => (
              <div key={stepNum} className="flex items-center flex-1">
                <div className="flex flex-col items-center flex-1">
                  ...
                  <p className={`text-xs mt-2 ${step >= stepNum ? 'text-white' : 'text-zinc-500'}`}>
                    {stepNum === 1 && 'Personal'}
                    {stepNum === 2 && 'Territory'}
                    {stepNum === 3 && 'Business'}
                    {stepNum === 4 && 'Review'}
