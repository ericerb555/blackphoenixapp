...
export default function EnterpriseEmailManagement() {
  ...
  const accountColumns: DataTableColumn<EmailAccount>[] = [
    ...
    },
    {
      ...
      header: 'Type',
      render: (row) => (
        <span className={`px-2 py-1 rounded text-xs font-bold border ${accountTypeColors[row.accountType]}`}>
      ...
      <div className="max-w-[1800px] mx-auto mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-4">
            ...
            <p className="text-xs text-blue-400 mt-1">Automatic creation enabled</p>
          </div>
              ...
              return (
                <div key={domain.id} className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-6 hover:border-[#3A3A3A] transition">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-1 rounded text-xs font-bold border ${statusColors[domain.status]}`}>
                            {domain.status.toUpperCase()}
                          </span>
                          <span className="px-2 py-1 bg-blue-600/20 text-blue-400 rounded text-xs font-bold border border-blue-500/30 flex items-center gap-1">
                            <ProviderIcon className="w-3 h-3" />
                            {domain.provider.replace('-', ' ').toUpperCase()}
                          </span>
                          {domain.autoProvision && (
                            <span className="px-2 py-1 bg-purple-600/20 text-purple-400 rounded text-xs font-bold border border-purple-500/30 flex items-center gap-1">
                              <Zap className="w-3 h-3" />
                              AUTO-PROVISION
                            </span>
                          )}
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-4">
                    ...
                    <div className="bg-[#0A0A0A] rounded-lg p-3 border border-[#2A2A2A]">
                      <p className="text-xs text-gray-400 mb-1">Storage Limit</p>
