...
/**
 * Work requests are spread over more than one key, so an update has to find the
 * record before it can write it back.
 *
 *   all_work_requests        real customer submissions — where live jobs land
 *   work_requests            what most property-management routes read/write
 *   work_requests_anonymous  completed jobs kept for the public gallery
 *
 * This route used to read and write `work_requests` unconditionally. That key
 * holds nothing, so every assignment attempt returned "Work request not found"
 * and no job has ever actually been assigned to anybody. Finding the record
 * first, and writing back to whichever list it came from, fixes that without
 * migrating the other routes.
 */
const WORK_REQUEST_KEYS = ['all_work_requests', 'work_requests', 'work_requests_anonymous'];
async function findWorkRequest(id: string): Promise<{ key: string; rows: any[]; index: number } | null> {
  for (const key of WORK_REQUEST_KEYS) {
    const rows = (await kv.get(key) as any[]) || [];
    if (!Array.isArray(rows)) continue;
    const index = rows.findIndex((item: any) => String(item?.id) === String(id));
    if (index >= 0) return { key, rows, index };
  }
  return null;
}

app.post('/make-server-3eae23a6/property-management/work-requests/:id/assign', async (c) => { try { const user = await requirePropertyAdmin(c); if (!user) return c.json({ success: false, error: 'Administrator access is required.' }, 403); const body = await c.req.json(); const crewName = String(body.crewName || '').trim().slice(0, 160); if (!crewName) return c.json({ success: false, error: 'Choose a crew or technician before assigning this request.' }, 400); const found = await findWorkRequest(c.req.param('id')); if (!found) return c.json({ success: false, error: 'Work request not found.' }, 404); const { key, rows, index } = found; const now = new Date().toISOString();
  // The crew name is kept because that is what the assign screen collects and
  // what existing records carry. An email is recorded alongside it when one is
  // given, because matching a timesheet to a job on a free-text name is fragile
  // and an address is not.
  const assignedToEmail = String(body.crewEmail || body.assignedToEmail || '').trim().toLowerCase().slice(0, 200);
  const assignedEmployeeId = String(body.employeeId || '').trim().slice(0, 100);
  const request = rows[index] = { ...rows[index], status: 'assigned', assignedTo: crewName, ...(assignedToEmail ? { assignedToEmail } : {}), ...(assignedEmployeeId ? { assignedEmployeeId } : {}), assignedAt: now, assignedBy: user.email, updated_at: now }; await kv.set(key, rows); const alerts = (await kv.get('admin_alerts') as any[]) || []; alerts.unshift(propertyAdminAlert(`Crew assigned: ${request.title || request.id}`, `${crewName} assigned by ${user.email}.`, { workRequestId: request.id, crewName })); await kv.set('admin_alerts', alerts.slice(0, 200)); return c.json({ success: true, data: request }); } catch (error: any) { return c.json({ success: false, error: error.message }, 500); } });
...
app.post('/make-server-3eae23a6/bid-room/notify-invites', async (c) => {
  try {
    ...
    // (005); anyone else gets nothing back and therefore notifies nobody.
    const { data: providers } = await asUser
      .from('organizations').select('id, name, email, phone').in('id', orgIds);
    if (!providers?.length) {
    ...
    const TWILIO_AUTH    = Deno.env.get('TWILIO_AUTH_TOKEN')   || '';
    const TWILIO_FROM    = Deno.env.get('TWILIO_PHONE_NUMBER') || '';
    const smsConfigured  = Boolean(TWILIO_SID && TWILIO_AUTH && TWILIO_FROM);
    const COMPANY_NAME = resolveCompanyName();
    ...
    const smsText =
      `${COMPANY_NAME}: you're invited to bid on "${request.title}"` +
      (request.trade ? ` (${request.trade})` : '') +
      `. Bids due ${due}. Submit your price: ${APP_URL}/bid-room`;

    const results: any[] = [];
    for (const p of providers) {
      const row: any = { org: p.name, email: false, sms: false };

      // ── email ────────────────────────────────────────────────────────────
      const to = String(p.email || '').trim();
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to)) {
        row.emailReason = 'no email on file';
      } else if (!RESEND_API_KEY) {
        row.emailReason = 'RESEND_API_KEY is not set';
      } else {
        ...
        try {
          ...
          if (!res.ok) {
            console.error('[bid-room/notify-invites] resend error:', await res.text());
            row.emailReason = 'email provider rejected the message';
          } else {
            row.email = true;
          }
        } catch (err: any) {
      ...
      // ── sms ──────────────────────────────────────────────────────────────
      // Independent of email on purpose: a provider with only one of the two on
      // file still gets told, and one channel failing does not suppress the
      // other. toE164() normalises the mixed formats the kv data carries
      // ("978-500-5475" and "6177100058" both appear).
      const phone = String(p.phone || '').trim();
      if (!phone) {
        row.smsReason = 'no phone on file';
      } else if (!smsConfigured) {
        row.smsReason = 'SMS is not configured on the server';
      } else {
        try {
          const res = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${TWILIO_SID}/Messages.json`, {
            method: 'POST',
          ...
          if (!res.ok) {
            console.error('[bid-room/notify-invites] twilio error:', await res.text());
            row.smsReason = 'carrier rejected the message';
          } else {
            row.sms = true;
          }
        } catch (err: any) {
          console.error('[bid-room/notify-invites] sms failed:', err);
          row.smsReason = err?.message || 'send failed';
        }
      }

      // `sent` means the provider was reached somehow — by either channel.
      row.sent = row.email || row.sms;
      results.push(row);
    }
