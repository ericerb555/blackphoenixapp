...
import {
  ShoppingBag, ShoppingCart, X, Search, Star, Tag,
...
async function processCheckout(items: CartItem[], email: string): Promise<boolean> {
  try {
    const res = await fetch(`${SERVER}/stripe/create-checkout`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${publicAnonKey}` },
      body: JSON.stringify({
        items: items.map(i => ({ id: i.product.id, name: i.product.title, price: i.product.price, qty: i.quantity })),
        email,
  ...
  // Stripe not yet configured — simulate success locally
  return false;
}
...
export default function PropertyMarketplace() {
  ...
  async function handleCheckout() {
    ...
    const stripeWorked = await processCheckout(cart, email);
    if (!stripeWorked) {
